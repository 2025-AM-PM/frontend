...
import { MiniBoardProps, Post } from '../types'; // Post 타입도 가져옵니다.
...
const MiniBoard: React.FC<MiniBoardProps> = ({ boardTitle, posts }) => { // posts 배열을 props로 받습니다.
                    ...
                    {posts.map((post) => (
                        ...
                        >
                            <td className="td-title">
                                ...
                                <a href={post.link} onClick={(e) => e.preventDefault()}>{post.title}</a>
                            </td>
                            <td className="td-author">{post.author}</td>
                            <td className="td-date">{formatTimeWithLibrary(post.createdAt)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
